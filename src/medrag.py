...
openai.base_url = "https://openrouter.ai/api/v1/chat/completions"
...
if openai.__version__.startswith("0"):
    ...
    if openai.api_type == "azure":
        ...
        openai_client = lambda **x: requests.post(
...
class MedRAG:
    ...
    def __init__(self, llm_name="OpenAI/gpt-3.5-turbo-16k", rag=True, follow_up=False, retriever_name="MedCPT", corpus_name="Textbooks", db_dir="./corpus", cache_dir=None, corpus_cache=False, HNSW=False):
        ...
        if self.llm_name.split('/')[0].lower() in ["openai", "deepseek"]:
            ...
            if "1.5" in self.llm_name.lower() or "2.0" in self.llm_name.lower():
            ...
            if "mixtral" in llm_name.lower():
                self.tokenizer.chat_template = open(
                    './templates/mistral-instruct.jinja').read().replace('    ', '').replace('\n', '')
                self.max_length = 32768
                self.context_length = 30000
                ...
                self.tokenizer.chat_template = open(
                    './templates/meditron.jinja').read().replace('    ', '').replace('\n', '')
                self.max_length = 4096
                self.context_length = 3072
    ...
    def generate(self, messages, **kwargs):
        ...
        if "openai" in self.llm_name.lower() or "deepseek" in self.llm_name.lower():
            try:
                ans = openai_client(
                    model=self.model if "openai" in self.llm_name.lower() else 'deepseek/deepseek-r1',
                    messages=messages,
                    temperature=0.6,
                    **kwargs
                )
        ...
        for i in range(max_iterations):
            ...
            if save_path:
                with open(save_path, 'w') as f:
                    json.dump([p if type(p) == dict else p.model_dump()
                              for p in saved_messages], f, indent=4)
            ...
            if i >= n_rounds and ("## Answer" in last_content or "answer is" in last_content.lower()):
                ...
                try:
                    ...
                    action_str = re.search(
                        r"output\": (\[.*\])", action_str, re.DOTALL).group(1)
                    action_list = [re.sub(r'^\d+\.\s*', '', s.strip())
                                   for s in eval(action_str)]
                except Exception as E:
                ...
                for question in action_list:
                    ...
                    try:
                        rag_result = self.medrag_answer(
                            question, k=k, rrf_k=rrf_k, **kwargs)[0]
                        context += f"\n\nQuery: {question}\nAnswer: {rag_result}"
    ...
    def generate_medical_case(self, disease, case_details=None, **kwargs):
        template_path = os.path.join(os.path.dirname(
            __file__), "../templates/medical_case.jinja")
